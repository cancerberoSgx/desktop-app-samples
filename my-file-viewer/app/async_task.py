...
"""Facade for running blocking calls (filesystem I/O against the folder the
user has open) on a background thread and delivering their outcome back on
the UI thread, so wx never freezes while waiting on disk/network-mounted
storage. Built on wx.lib.delayedresult (thread + wx.CallAfter under the
hood) - copied verbatim from my-redis-viewer, which this project was
templated from; it's generic infrastructure with nothing filesystem-specific
in it. See FolderExplorerPage._load_folder for the intended usage, and reuse
the same AsyncTaskRunner instance for any new blocking action a page adds -
this is the pattern every future FileSystemService action (rename, delete,
copy, recursive size, glob search, ...) must go through too.
"""
...
class AsyncTaskRunner:
    ...
    offers:
        ...
        self._async.run(
            ...
            on_success=lambda listing: self._render(listing),
            on_error=lambda exc: wx.MessageBox(f"Failed: {exc}"),
            disable=[self._open_btn],
        )
    """
    ...
    def run(
        ...
        """Run `work` (a zero-arg callable that may block, e.g. a
        filesystem call) on a background thread.
        ...
        ignored, so a stray double-click can't start overlapping filesystem
        actions against the same page.
