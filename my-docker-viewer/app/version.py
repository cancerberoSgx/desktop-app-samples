...
def get_version() -> str:
    ...
    VERSION (kept in sync with the aur/PKGBUILD's pkgver by
    scripts/bump-app-version.py) is the only thing both a source checkout
    and a frozen build can read this from.
    """
    try:
