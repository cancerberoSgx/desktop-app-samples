...
"""Facade for running blocking calls (DuckDB/SQLite/Postgres queries, file
inference, ...) on a background thread and delivering their outcome back on
the UI thread, so wx never freezes while a datasource is being read. Built
on wx.lib.delayedresult (thread + wx.CallAfter under the hood) - see
DatasourcesPage._connect for the intended usage, and reuse the same
AsyncTaskRunner instance for any new blocking action a page adds.

This is the right tool for routine, page-scoped background loads (test a
connection, list tables/columns, infer CSV/JSON types, ...) that don't need
a visible status or to be cancelable. For long, deliberately-triggered
actions the user should be able to see running and cancel - currently
exporting and running a SQL script - see TaskManager in app/task_manager.py
instead, which adds a global (not per-page) single-flight status and
cancellation on top of the same delayedresult plumbing.
"""
...
class AsyncTaskRunner:
    ...
    def run(
        ...
        """Run `work` (a zero-arg callable that may block, e.g. a query
        against a datasource) on a background thread.
        ...
        ignored, so a stray double-click can't start overlapping queries
